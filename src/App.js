...
const bld=t=>{const p=t.split(/\*\*(.*?)\*\*/);return p.map((x,j)=>j%2===1?<strong key={j} style={{color:C.tx}}>{x}</strong>:<span key={j}>{x}</span>)};
const fixU=t=>t.replace(/\\u([0-9a-fA-F]{4})/g,(_,h)=>String.fromCharCode(parseInt(h,16)));
const rM=t=>t.split("\n").map((l0,i)=>{const l=fixU(l0);
...
if(l.match(/^[\u2705\u2714\u2611]/)||l.startsWith("✅"))return <div key={i} style={{display:"flex",gap:6,margin:"3px 0",padding:"5px 8px",background:C.gnd+"66",borderRadius:4,borderLeft:"2px solid "+C.gn+"66",fontSize:11,color:C.gn,lineHeight:1.5}}><span style={{flexShrink:0}}>✅</span><span style={{color:C.ts}}>{bld(l.replace(/^[\u2705\u2714\u2611✅]\s*/,""))}</span></div>;
if(l.match(/^[\u26a0\u26a0\ufe0f]/)||l.startsWith("⚠"))return <div key={i} style={{display:"flex",gap:6,margin:"3px 0",padding:"5px 8px",background:C.amd+"44",borderRadius:4,borderLeft:"2px solid "+C.am+"66",fontSize:11,color:C.am,lineHeight:1.5}}><span style={{flexShrink:0}}>⚠️</span><span style={{color:C.ts}}>{bld(l.replace(/^[\u26a0\ufe0f⚠️]\s*/,""))}</span></div>;
...
const TABS=[{id:"dash",l:"Dashboard",i:"\u26a1"},{id:"tickets",l:"Tickets",i:"\ud83d\udccb"},{id:"brief",l:"Brief",i:"\ud83d\udcca"},{id:"trends",l:"Timeline",i:"\ud83d\udcc8"},{id:"health",l:"Health",i:"\ud83d\udc8a"},{id:"crm",l:"CRM",i:"\ud83d\udcac"},{id:"ask",l:"Ask \u2726",i:"\u2726"}];
...
<main style={{flex:1,padding:14,overflowY:"auto"}}>{tab==="dash"&&<DashTab onNav={setTab}/>}{tab==="tickets"&&<TicketsTab/>}{tab==="brief"&&<BriefTab/>}{tab==="trends"&&<TrendsTab/>}{tab==="health"&&<HealthTab/>}{tab==="crm"&&<CrmTab/>}{tab==="ask"&&<AskTab/>}</main>
